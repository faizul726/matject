...
powershell -ExecutionPolicy Bypass -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%1'); $s.TargetPath = '%cd%\matject.bat'; $s.Arguments = 'placebo' ; $s.IconLocation = '%cd%\.settings\matject_icon.ico'; $s.Save()"
goto :EOF
