@echo off
if not defined murgi echo [41;97mYou can't open me directly[0m :P & cmd /k

title %title% ^(help^)
cls
echo !WHT!How to use %title%?!RST!
echo.
echo.
echo !YLW!Q. How to use it?!RST!
echo A. This is a Text-based user interface.
echo    Means you have to navigate using keyboard buttons.
echo    All actions are labeled with what key to press.
echo    Everything is self explanatory. So, just read what it says -,-
echo.
echo !YLW!Q. Do I need IObit Unlocker to be installed?!RST!
echo A. Yes.
echo.
echo !YLW!Q. Do I need to disable anti-virus/encryption?!RST!
echo A. Yes, especially ransomware protection as it might make Matject unable to read/write files.
echo    If you don't want to then you can allow full access to folders that are mentioned in FAQ of README.MD
echo.
echo !YLW!Q. Does it work on all version?
echo A. Maybe.
echo.
echo !YLW!Q. It's not working, what to do?!RST!
echo A. Join !CYN!discord.gg/yss!RST! and send message in #windows or #renderdragon.
echo    I will help.
%backmsg%
